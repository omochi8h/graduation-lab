...
t = []
c.execute("select compiletime from seito where seitoname=? and comp=1",("学習者D",))
for i in c:
    t.append(i["compiletime"])
ti = t[-1]
ti = ti + 73800
c.execute("update seito set compiletime=? where seitoname=? and comp = 1",(ti,"学習者D"))
conn.commit()
...
class ScrollTable(QWidget):
    def __init__(self, parent):
        ...
        for seito in sorted(seitoset):
            ...
            achieve = [0]
            if kadaiidentify == 0:
                c.execute("select*from seito where seitoname=? and comp=1", (seito,))
            else:
                c.execute("select kadainame from kadai")
                for i in c:
            ...
            row.append(seito)
            row.append(kadai[-1])
            row.append(round((time.time()-jikan[-1])/60))
            row.append(judge[-1])
...
class SeitoDetail(QFrame):
    def __init__(self, parent=None):
        ...
        if seitoidentify != "":
            ...
            label4 = QLabel("判定値 ： " + str(round(judge[-1], 3)))
            font = QFont()
            font.setPointSize(13)
            label4.setFont(font)
            ...
            grid.addWidget(label5,2,0,1,6)
            grid.addWidget(label6,3,0,1,6)
            grid.addWidget(button3,3,4,1,1)
            grid.addWidget(button4,3,5,1,1)
            grid.addWidget(self.edit1,4,0,1,6)
            grid.addWidget(label7,5,0,1,6)
            grid.addWidget(edit2,6,0,1,6)
            grid.addWidget(button1,7,0,1,3)
            grid.addWidget(button2,7,3,1,3)
            self.setLayout(grid)
